...
Summarise current asking-market conditions for <municipality>, from listings only. entity_stats on listings is SECTION-based: each call computes ONE curated section id and costs 0.50 credits (the 3 calls below = 1.50 credits total). Listings section filters accept ONLY municipality / district / canton / postal_code / bbox + property_type (an ARRAY) + deal_type ("sale" | "rent", REQUIRED) + date_from / date_to (ISO with T+Z) + active (boolean) — no rooms/price filters. For CURRENT market levels add active: true; for a recent-window read add date_from (e.g. 12 months back); omit both for the full 2006–present history. Follow these steps exactly:
...
4. THE DOCUMENT
   ...
   Do NOT include registered land-registry transactions — this overview is asking-market only. Read the latest quarters for the current picture — section figures span the full listing history. Notes-line material: sparse data, asking prices are advertised, not realised. Next-step candidates: /popety:find-development-sites to source under-exploited parcels, or /popety:investment-yield for a specific property — offer the more relevant one.
